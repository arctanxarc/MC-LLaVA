...
def get_test_image_paths_by_concept(dataset_root, concept_name):
    ...
    concept_test_image_paths = [os.path.join(concept_test_images_path, x) for x
                                ...
                                if x.lower().endswith(('.png', '.jpeg', '.jpg')) and "som" in x]
...
def get_test_image_paths_by_concept_for_neg(dataset_root, concept_name):
    concept_case = data_w_case[concept_name]
    concept_test_images_path = os.path.join(dataset_root, concept_case, "concept", "test", concept_name)
    concept_test_image_paths = [os.path.join(concept_test_images_path, x) for x
                                in os.listdir(concept_test_images_path)
...
def get_test_multi_image_paths_by_concept_for_neg(dataset_root, concept_name):
    concept_case = data_w_case[concept_name]
    multi_path = os.path.join(dataset_root, concept_case, "multi")
    multi_image_paths = []
    friends = [concept_name]
...
class TestDatasetForSingleRec(Dataset):
    def __init__(
        ...
        for _ in range(num_single_neg):
            ...
            neg_single_image_paths.append(random.choice(get_test_image_paths_by_concept_for_neg(self.dataset_root, neg_concept_name)))
        ...
        for _ in range(num_multi_neg):
            ...
            neg_imgs_list = get_test_multi_image_paths_by_concept_for_neg(self.dataset_root, neg_concept_name)[1]
            neg_multi_image_paths.append(random.choice(neg_imgs_list))
...
class TestDatasetForMultiRec(Dataset): #
    def __init__(
        ...
        concept_numbers = {k: v for k, v in zip(concept_names, NUMBERS)}
        ...
        for concept_name in concept_names:
            ...
            box_prompts += [f"Mark number {concept_numbers[concept_name]} is <{concept_name}>."] * len(tmp_single_image_paths)
            ...
            for concept in concept_names:
                this_box_prompt += f"Mark number {concept_numbers[concept]} is <{concept}>. "
            box_prompts += [this_box_prompt] * len(tmp_multi_image_paths)
        ...
        # assert len(pos_image_paths) == len(concept_names) * 5 + len(concept_names) * 5 + 5, f"So far, num of positive images should be 5 * m + 5 * m + 5, but got {len(pos_image_paths)}" # ZK
        ...
        for curr_concept_name in concept_names:
            for other_concept_name in concept_names:
                ...
                box_prompts += [f"Mark number {concept_numbers[other_concept_name]} is <{other_concept_name}>."] * len(temp_neg_image_paths)
        ...
        if self.more_negative:
            for _ in range(num_single_neg):
                ...
                neg_image_paths.append(random.choice(get_test_image_paths_by_concept_for_neg(self.dataset_root, neg_concept_name)))
            ...
            for _ in range(num_multi_neg):
                ...
                neg_image_paths.append(random.choice(get_test_multi_image_paths_by_concept_for_neg(self.dataset_root, neg_concept_name)[1]))
    ...
    def __getitem__(self, i):
        ...
        if "_" not in quary_name:
            example["query"] = box_prompt + f'Can you see <{quary_name}> in this photo? Answer the question using a single word Yes or No.'
        else:
            concept_names = quary_name.split("_")
            concept_names = [f'<{x}>' for x in concept_names]
            example["query"] = box_prompt + f'Can you see {" and ".join(concept_names)} in this photo? Answer the question using a single word Yes or No.'
...
class TestDatasetForMultiVQA(Dataset): #
    def __init__(
        ...
        # types = ["si_sq", "mi_sq", "mi_mq"]
        # type = ["vqa", "choice"]
        self.number2name = {idx+1:name for idx, name in enumerate(self.concept_names)}
        ...
        for concept_name in concept_names:
            ...
            for image_path, qa in qa_data.items():
                # .../0.png --> .../0_bboxes.png or jpg
                # base, ext = os.path.splitext(image_path)
                # if ext.lower() in ['.png', '.jpg', '.jpeg']:
                #     image_path = f"{base}_som{ext}"
                self.image_paths.append(image_path)
                
                self.som_prompts.append("")
                quary = qa['Human']
                
                self.quaries.append(quary)
        ...
        try:
            with open(curr_case_qa_file) as f:
                ...
                # print(curr_case_qa_data)
        except:
            raise ValueError(f"Failed to load {curr_case_qa_file}")
        
        som_info_path = os.path.join(multi_cases_dir, self.case, "som_info.json")
        try:
            with open(som_info_path) as f:
                som_info = json.load(f)
        except:
            raise ValueError(f"Failed to load {som_info_path}")
        ...
        for image_path, qa_pairs in curr_case_qa_data.items():
            for i, qa in enumerate(qa_pairs):
                ...
                base, ext = os.path.splitext(image_path)
                if ext.lower() in ['.png', '.jpg', '.jpeg']:
                    tmp_image_path = f"{base}_som{ext}"
                self.image_paths.append(tmp_image_path)
                
                self.answers.append(qa['AI'])
                som_prompt = ""
                if som_info[image_path] != {}:
                    for concept_number, pos in som_info[image_path].items():
                        number = int(concept_number)
                        concept = self.number2name[number]
                        som_prompt += f"<{concept}> is located at 'Mark Number {number}'. "
                self.som_prompts.append(som_prompt)
                quary = qa['Human']
                self.quaries.append(quary)
    ...
    def __getitem__(self, i):
        ...
        if self.type == "vqa":
            ...
            example["query"] = self.quaries[i] + "\nIMPORTANT: Answer the question using a single letter."
        else:
            raise ValueError(f"Unsupported type: {self.type}")
        example["som_prompt"] = self.som_prompts[i]
        example["answer"] = self.answers[i]
        example["type"] = self.types_query[i]
        example['image_sizes'] = image_sizes
        example['has_image'] = True
...
class TestDatasetForVG(Dataset):
    def __init__(
        self,
        dataset_root,   #
        concept_names,  # ["sks1", "sks2"]
        ...
        # k: xx/xxx/0.png -> 0.png
        # som_info = {os.path.basename(k): v for k, v in som_info.items()}
        
        curr_case_qa_file = os.path.join(multi_cases_dir, self.case, "vqa_position.json")
        try:
            with open(curr_case_qa_file) as f:
                curr_case_qa_data = json.load(f)
                # print(curr_case_qa_data)
        ...
        for image_path, qa_pairs in curr_case_qa_data.items():
            # xx/yy/0.png (jpg) -> xx/yy/0_som.png (jpg)
            ext = os.path.splitext(image_path)[1]
            som_image_path = image_path.replace(ext, f"_som{ext}")
            for i, qa in enumerate(qa_pairs):
                self.image_paths.append(som_image_path)
                som_prompt = ""
                if som_info[image_path] != {}:
                    for concept_number, pos in som_info[image_path].items():
                        number = int(concept_number)
...
class TestDatasetForSingleVQA(Dataset):
    def __init__(
        self,
        dataset_root,   
        concept_name,  # "sks1"
...
class TestDatasetForCaptionGeneration(Dataset):
    def __init__(
        ...
        for concept_name in concept_names:
            ...
            tmp_name = concept_name.split('_')[0]
            self.quaries += [f'<{tmp_name}>'] * len(concept_single_image_paths)
...
class TestDatasetForTextOnlyQAChoice(Dataset):
    def __init__(
        self,
        dataset_root,   # /home/hpyky/MulBench
        concept_names,  # ["sks1", "sks2"]
    ):
        self.dataset_root = dataset_root
        self.concept_names = concept_names
...
class TestDatasetForSingleTextOnlyQAChoice(Dataset):
    def __init__(
        self,
        dataset_root,   # /home/hpyky/MulBench
        concept_name,  # "sks1"
...
if __name__ == "__main__":
    ...
    cases_file = "/home/hpyky/MulBench/cases.json"
    ...
    train_set = TrainingDataset("/home/hpyky/mulllava/generate_training_data", ["Hai", "Jun"], model.device, model.config, image_processor)
    print(len(train_set))